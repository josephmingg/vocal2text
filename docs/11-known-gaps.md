...
| G13 | **Burmese recognition runs on Whisper, which is poor at it** (80–100% WER). The text layer, detection, dictionary, and formatting are complete; only the engine is not. `availability(for: .burmese)` reports `.readyWithCaveat` and both apps say so plainly via `BurmeseSupportNote` | docs/04 App. A | **Benchmark done** (docs/benchmarks/burmese-asr-2026-08-17.md): Omnilingual CTC int8 measured at 10.78% CER (1B) / 15.19% (300M) on FLEURS `my_mm` — GO. Next: sherpa-onnx adapter behind `TranscriptionEngine`, Mac/1B first; iPhone call after on-device RTF |
