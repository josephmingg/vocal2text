...
| G10 | ~~iOS app not yet built~~ — built. Main app (M6) plus the M7 keyboard, share-sheet import, and Live Activity all compile in CI; **none of it has run on a phone yet** | docs/02 | Device pass per docs/14; AC-i1/i3/i4/i5/i6 stay open until then |
...
| G15 | Darwin-notification delivery latency to a backgrounded, audio-active app is unmeasured — the docs/02 §3.1 `[verify]` spike | docs/02 §3.1 | Measure on device per docs/14 §3; commit to `docs/benchmarks/` |
| G16 | An armed capture session holds a live input tap for its whole window, so the orange indicator stays lit and battery is spent. Deliberate (it is the only residency mechanism iOS offers) and disclosed in the arming UI, but the battery cost is unquantified | FR-i1.3 / NFR-i3 | docs/14 §7 |
